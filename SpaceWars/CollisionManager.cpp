...
void CollisionManager::smallEnemySpaceShipHitSpaceShip(std::unique_ptr<Enemyships>& enemySmallShip)
{
	if (!enemySmallShip->isDestructionAnimationBegan()) {
		enemySmallShip->damage(3);
		hitSpaceShipEffect();
...
//Function to detect if any of space ship projectiles
//Hit an asteroid, if so it decreases the asteroids health
...
//Function to detect if any asteroids collided with spaceShip
...
//Function to detect if enemy projectile collided with spaceShip
...
//Functions to detect collision of a powerup pick up and the player spaceship
...
//Function to detect if the player hit the enemy Ship
...
//Functions to detect collision between two bullets
...
//manages all collision detections.
