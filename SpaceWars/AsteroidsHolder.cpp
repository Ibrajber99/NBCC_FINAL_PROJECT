...
{
	std::for_each(std::execution::par,asteroidsContainer.begin(),
...
{
	std::for_each(asteroidsContainer.begin(), asteroidsContainer.end(),
		[&](auto &asteroid) {
			return asteroid->drawAsteroid(window);
		});
...
void AsteroidsHolder::moveAsteroids()
{
	std::for_each(std::execution::par,
		...
		[&](auto& asteroid) {
			...
			return movementFlag ? asteroid->moveAsteroid
			(sf::seconds(1.f / 60.f).asSeconds())
...
void AsteroidsHolder::destroyAsteroids()
{
	std::for_each(std::execution::par, asteroidsContainer.begin(),
		asteroidsContainer.end(), [](auto& asteroid) {
			return asteroid->setIsDestroyed(true);
		});

}

void AsteroidsHolder::generateSpawnPositions(size_t size) {
	spawnGrid.width = window.getSize().x 
		/ largeAsteroidTex.getSize().x;

	spawnGrid.height = window.getSize().y
		/ largeAsteroidTex.getSize().y;
