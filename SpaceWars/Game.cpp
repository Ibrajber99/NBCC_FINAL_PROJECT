...
//Preparing essential game tools 
Game::Game()
...
//Function to intialize the space ship position and other
//few sprites for the Game UI
...
//Fucntion that starts a new instance of the game
void Game::resetGame()
{
...
//Key listening fucntion 
// if left courser is pressed bullet shoots out of the space ship
...
//Key listening function if Escape button is pressed 
//the game state will be changed to paused 
...
//activates functions for when a key press event is registered
...
//draws everything for the game and displays the window 
...
//renders game objects on the scene
void Game::render()
{
	window.clear();
...
void Game::updateMessagepopUp()
{
	...
	if (GAME_STATE == STATE::PLAYING) {
		gameStateText.setPosition(sf::Vector2f(window.getSize().x / 2, 0.0f));
